...
class Week21JeongjySolutionTest {
    ...
    @DisplayName("할인 행사")
    @ParameterizedTest
    @MethodSource("discountEventProvider")
    void discountEventTest(String[] want, int[] number, String[] discount, int expected) {
        // given & when
        int actual = solution.discountEvent(want, number, discount);

        // then
        assertThat(actual).isEqualTo(expected);
    }

    private static Stream<Arguments> discountEventProvider() {
        return Stream.of(
